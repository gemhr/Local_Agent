...
| `LOCAL_AGENT_RETRIEVAL_STRATEGY` | lifespan composition | enum | `BASELINE` | `BASELINE`,`HYBRID_RRF` | no | APPLICATION_SCOPE | yes | public-safe enum | 默认 `BASELINE`；`HYBRID_RRF` 需要完整 v2 generation，且 WP1 边界内以 `RETRIEVAL_STRATEGY_NOT_IMPLEMENTED` 安全失败（不回退 baseline） | `BASELINE` |
