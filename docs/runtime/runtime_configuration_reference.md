...
PRODUCTION 的 `LOCAL_AGENT_API_HOST` 必须是 numeric loopback（IPv4 `127.0.0.0/8` 或 `::1`）；derived IPv6 URL使用brackets。`LOCAL_AGENT_API_BASE_URL` 必须为 `http`、numeric loopback、无userinfo/query/fragment，path仅empty或`/`。LOCAL/TEST不强制loopback，但仍不提供 inbound TLS；所有 profile 的 `/api/*` 都经过 Stage6 EdDSA JWT、PostgreSQL Principal 与 Redis limiter 边界。
...
| `LOCAL_AGENT_TRACING_ENABLED` | ObservabilityService | strict bool | `0` | `1`,`0`,`true`,`false` | no | PROCESS_SCOPE | yes | public-safe flag | false 时不创建 SDK provider；true 且无 exporter 时仍生成进程内 span，但不外发 | `0` |
| `LOCAL_AGENT_OTEL_SERVICE_NAME` | OTel Resource | string | `localagent-api` | non-empty，≤128 chars | no | PROCESS_SCOPE | yes | low-cardinality resource | 非法值 fail closed；独立进程后续使用各自 service name | `localagent-api` |
| `LOCAL_AGENT_OTEL_EXPORTER_OTLP_ENDPOINT` | OTLP/HTTP exporter | URL string | empty | operator-provided OTLP base 或 `/v1/traces` endpoint | no | PROCESS_SCOPE | yes | sensitive operator config（`repr=False`） | empty=不构造 exporter；导出失败不改变业务结果 | `http://otel-collector:4318` |
| `LOCAL_AGENT_OTEL_TRACE_SAMPLE_RATIO` | OTel sampler | finite float | `0.1` | 0..1 | no | PROCESS_SCOPE | yes | public-safe ratio | invalid value fails load；使用 ParentBased + TraceIdRatioBased | `0.1` |
| `LOCAL_AGENT_HEALTH_CHECK_TIMEOUT_SECONDS` | ComponentReadinessService | finite float | `1.0` | >0 | no | PROCESS_SCOPE | yes | internal config | PG/Redis/Kafka 单项 check 的 bounded timeout；invalid value fails load | `1.0` |
...
| `LOCAL_AGENT_MAX_ACTIVE_RUNS` | RunExecutionSupervisor | int | `12` | integer ≥1 | no | APPLICATION_SCOPE | yes | internal config | in-flight Run admission capacity；与 blocking executor capacity 独立；越界显式值 fail closed | `12` |
