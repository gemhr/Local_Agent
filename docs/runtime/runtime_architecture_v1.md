...
- **Permission**：per-Tool explicit allowed Agent IDs（当前 7 Tool × 5 Agent = 35 条显式授权关系，无 implicit default allow）；未知 principal / missing policy / explicit deny 均 fail closed。
...
- **Approval**：effective risk >= policy threshold（HIGH）-> `APPROVAL_REQUIRED`。在有 run-scoped `ToolApprovalController` 的 COORDINATED active Run 中，既有 `TOOL_APPROVAL_REQUESTED -> WAITING_FOR_APPROVAL -> APPROVE/REJECT -> execution claim` 路径拥有审批事实；controller 缺失或审批证据无法可靠发布时仍 fail closed、零执行。HTTP approve/reject 只是命令 transport，不成为 approval truth；Stage6 在 transport 提供 JWT Principal、RBAC 与 Run ownership，仍无 durable pause/resume 或 reconnect/replay。
...
- 该测试证据不改变现有 Owner、contract classification 或安全能力；它不代表 durable approval resume、filesystem/path authorization、sandbox 或 dedicated governance event 已实现。run-scoped human approval transport 已由后续阶段实现；Stage6 HTTP 边界提供 JWT、Principal、RBAC 与 object ownership，但仍无 reconnect 或 durable pause-resume。
...
Stage5-Phase6-WP1/WP2（检索 provenance 与 Hybrid runtime）：Chroma marker 演进到 v2（`localagent_collection_contract_version=2`）。v2 增加 `generation_id`、`provenance_contract_version`、`provenance_sha256`、`corpus_id`、`source_manifest_sha256`、`chunk_policy_sha256`、`chunk_manifest_sha256`、document/chunk counts、`embedding_asset_tree_sha256`。v1 collection 对 BASELINE 仍合法；对 HYBRID_RRF 一律 REBUILD_REQUIRED（不自动迁移、不从 chunk rows 推断 provenance）。`LOCAL_AGENT_RETRIEVAL_STRATEGY`（默认 BASELINE）在 lifespan 捕获一次；HYBRID_RRF 需要完整 v2 generation，校验器保留已加载 BM25 index 并由唯一 lifespan composition root 注入独立 Hybrid adapter。Hybrid 使用一个合并 Dense channel 与唯一 BM25 sparse channel，串行在既有 `RERANK` slot 执行 RRF；没有 Chroma keyword、单通道 RRF、Dense score threshold 或 baseline fallback。BM25-only 候选必须向 active Dense collection 精确回取正文与 citation metadata。完整描述见 `runtime_operations_runbook.md` 的 KB Generation Build / Rollback Runbook。
