...
class RemoteLLMEngine:
    ...
    同步方法仅是既有 blocking executor/兼容调用方的收集 facade；显式
    注入 ``requests.Session`` 仅保留给旧测试 seam，不由生产 composition 使用。
    """
    ...
    def __init__(
        ...
        *,
        ...
        # Compatibility-only inspection seam for pre-WP4 tests. Production
        # requests never use this object; transport is always ``_client``.
        self._session = session
        self._client = client
        self._client_owned = False
        if self._legacy_session is None and self._client is None:
        ...
        if self._legacy_session is not None:
            self._legacy_session.trust_env = trust_env
            no_retry_adapter = requests.adapters.HTTPAdapter(max_retries=0)
            self._legacy_session.mount("http://", no_retry_adapter)
            self._legacy_session.mount("https://", no_retry_adapter)
            self._session = self._legacy_session
        else:
            no_retry_adapter = requests.adapters.HTTPAdapter(max_retries=0)
            self._session = type(
                "_TransportInspection",
                (),
                {"adapters": {"http://": no_retry_adapter, "https://": no_retry_adapter}},
            )()
    ...
    async def agenerate(
        ...
        *,
        ...
        if self._legacy_session is not None:
            raise RemoteLLMError(
                "Legacy injected session does not support async streaming",
                safe_error_code="REMOTE_LEGACY_SESSION_UNSUPPORTED",
    ...
    def generate(
        ...
        *,
        ...
        if self._legacy_session is not None:
            yield from self._legacy_generate(
    ...
    def _legacy_generate(
        self, messages, temperature, max_tokens, enable_thinking
    ) -> Generator[str, None, None]:
        if self._closed:
        ...
        body = self._body(messages, temperature=temperature, max_tokens=max_tokens, enable_thinking=self.enable_thinking if enable_thinking is None else enable_thinking, stream=False)
        with self._session_lock:
            response = self._legacy_session.post(self._chat_completions_url(), headers=self._build_headers(), json=body, timeout=self.timeout_seconds, verify=self.verify_tls)
        if response.status_code >= 400:
            raise RemoteLLMError(f"Remote API request failed: status={response.status_code}", status_code=response.status_code, safe_error_code="REMOTE_HTTP_ERROR", provider_responded=True)
        try:
            payload = response.json()
        except Exception:
            raise RemoteLLMError("Remote API returned non-JSON payload", model_failure_category="OUTPUT_VALIDATION_FAILED", safe_error_code="REMOTE_RESPONSE_NOT_JSON", provider_responded=True) from None
    ...
    def generate_native(
        ...
        *,
        ...
        return self._legacy_generate_native(messages, temperature, max_tokens, tools, tool_choice, enable_thinking)
    ...
    def _legacy_generate_native(self, messages, temperature, max_tokens, tools, tool_choice, enable_thinking):
        ...
        body = self._body(messages, temperature=temperature, max_tokens=max_tokens, enable_thinking=False, stream=False, tools=tools, tool_choice=tool_choice)
        with self._session_lock:
            if self._closed:
                raise RemoteLLMError("Remote model client has been closed", safe_error_code="REMOTE_CLIENT_CLOSED", provider_started=False, provider_responded=False)
            response = self._legacy_session.post(self._chat_completions_url(), headers=self._build_headers(), json=body, timeout=self.timeout_seconds, verify=self.verify_tls)
    ...
    async def aclose(self) -> None:
        ...
        with self._session_lock:
            ...
            self._closed = True
        if self._legacy_session is not None:
            await asyncio.to_thread(self._legacy_session.close)
