...
def _review(row): return BusinessReview(row.review_id, row.mission_id, ReviewType(row.review_type), row.subject_version, row.subject_digest, ReviewStatus(row.status), row.created_at, row.decided_at, row.decided_by, row.decision_comment)
...
class MissionService:
    ...
    async def get_mission(self, mission_id):
        ...
        return _mission(row)
    async def transition_mission(self, mission_id, to_status: MissionStatus | str, expected_version: int):
...
class BusinessReviewService:
    def __init__(self, database): self.database = database
    async def create_review(self, mission_id, review_type, *, subject_version=None, subject_digest=None, review_id=None):
        try: kind = ReviewType(review_type)
        except ValueError as exc: raise Stage8ValidationError("invalid review_type") from exc
        async with self.database.transaction() as session:
            if await repo.get_mission(session, mission_id) is None: raise Stage8NotFoundError("mission not found")
            row = await repo.add_review(session, dict(review_id=review_id or uuid.uuid4().hex, mission_id=mission_id, review_type=kind.value, subject_version=subject_version, subject_digest=subject_digest, status=ReviewStatus.PENDING.value))
            return _review(row)
    async def get_review(self, review_id):
        ...
        return _review(row)
    async def _decide(self, review_id, mission_id, target, *, decided_by=None, comment=None, subject_version=None, subject_digest=None):
        async with self.database.transaction() as session:
            ...
            if (row.subject_version is not None or row.subject_digest is not None) and (
                row.subject_version,
                row.subject_digest,
            ) != (subject_version, subject_digest):
