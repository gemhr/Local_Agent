(() => {
  ...
  function enableWorkflow() {
    ...
    $('observe-execution').disabled = !jobRunning;
  }
  ...
  async function decideReview(kind, button) { setBusy(button, true); try { const result = await api(`/api/stage8/reviews/${state.review.review_id}/${kind}`, { method: 'POST', body: JSON.stringify({ mission_id: state.missionId, subject_id: state.review.subject_id, subject_version: state.review.subject_version, subject_digest: state.review.subject_digest, decided_by: 'web-demo' }) }); state.review = result; $('review-meta').textContent = `Review ${result.status} · subject ${result.subject_id} · v${result.subject_version} · digest ${result.subject_digest}`; const currentMission = await api(`/api/stage8/missions/${state.missionId}`); const targetStatus = kind === 'approve' ? 'READY_FOR_EXECUTION' : 'CONTEXT_READY'; const transitioned = await api(`/api/stage8/missions/${state.missionId}/transition`, { method: 'POST', body: JSON.stringify({ status: targetStatus, expected_version: currentMission.version }) }); renderMission(transitioned); toast(`Business Review ${result.status}`); } catch (error) { toast(error.message); } finally { setBusy(button, false); enableWorkflow(); } }
  ...
  $('start-execution').addEventListener('click', async (event) => { const button = event.currentTarget; setBusy(button, true); try { const scenarioId = state.plan?.scenarios?.[0]?.scenario_id; if (!scenarioId) throw new Error('Reviewed TestPlan has no executable scenario'); const artifacts = await api(`/api/stage8/missions/${state.missionId}/cases/generate`, { method: 'POST', body: JSON.stringify({ scenario_ids: [scenarioId] }) }); const artifact = artifacts.find((item) => item.scenario_id === scenarioId); if (!artifact?.artifact_id) throw new Error('Case Platform did not return a canonical artifact'); state.job = await api(`/api/stage8/missions/${state.missionId}/execution`, { method: 'POST', body: JSON.stringify({ generated_case_artifact_id: artifact.artifact_id }) }); show('job-result', state.job); renderMission(await api(`/api/stage8/missions/${state.missionId}`)); toast(state.job.status === 'WAITING_FOR_RESOURCE' ? 'Waiting for a compatible free environment' : 'External Execution Job started'); } catch (error) { toast(error.message); } finally { setBusy(button, false); enableWorkflow(); } });
  $('observe-execution').addEventListener('click', async (event) => { const button = event.currentTarget; setBusy(button, true); try { const result = await api(`/api/stage8/execution-jobs/${state.job.job_id}/observe`, { method: 'POST', body: JSON.stringify({}) }); state.job = result; show('job-result', result); renderTriage(result); renderMission(await api(`/api/stage8/missions/${state.missionId}`)); toast('Execution observation completed'); } catch (error) { toast(error.message); } finally { setBusy(button, false); enableWorkflow(); } });
